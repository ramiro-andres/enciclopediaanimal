...
test.describe('Enciclopedia Animal — flujos E2E sin servidor', () => {
  ...
  test('vuelta desde enfermedad al glosario mediante término relacionado', async ({ page }) => {
    ...
    if (await terminoGlosario.count()) {
      // showView hace scroll al inicio; el scroll suave puede dejar scrollY
      // fraccional un instante — forzar tope y no exigir igualdad exacta.
      await page.waitForFunction(() => window.scrollY < 1, null, { timeout: 15_000 });
